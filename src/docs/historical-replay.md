...
Run these commands from the main `src` folder. The integrated dashboard includes Historical AIS replay.
...
## Integrated reporting corrections

The main app retains known queued vessels in its hourly queue forecast when the
solver defers them. Deferral is not disappearance. Started vessels retain a
representative berth and crane allocation derived from pre-cutoff observations;
remaining work is explicitly calibrated, not historical terminal telemetry.
Ambiguous terminal proxy matches select one latest observed call per proxy berth.
A validated schedule reports zero constraint violations even when demand is
unscheduled. Failed schedule validation is reported as one validation failure,
not as an enumerated count of every possible constraint violation.
Evaluation refuses missing hourly truth instead of filling gaps with zero
congestion. The dashboard only shows evaluation that matches the latest plan.
