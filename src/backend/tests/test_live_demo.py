...
def test_live_demo_expands_a_selected_port_to_the_overlapping_approved_scope(live_api):
    api=live_api
    created=api.post('/api/v1/optimisation/run',json={'as_of':EPOCH,'port_ids':['P01','P02','P03','P04'],
        'predictor':'baseline','time_limit_seconds':1})
    assert created.status_code==201,created.text
    plan=created.json()['plan']
    reviewed=api.post('/api/v1/plans/'+plan['id']+'/review',json={'expected_revision':1,'actor':'Supervisor'})
    assert reviewed.status_code==200,reviewed.text
    # The multi-port fixture intentionally exceeds the short test horizon.  The
    # scope test concerns approval coverage, so retain its computed assignments
    # while marking the publication complete for this transition setup.
    with api.app.state.sessions.begin() as db:
        run=db.get(m.OptimisationRun,created.json()['id'])
        run.diagnostics={**run.diagnostics,'unscheduled_call_ids':[],'pending_departure_call_ids':[]}
    approved=api.post('/api/v1/plans/'+plan['id']+'/approve',json={'expected_revision':2,'actor':'Supervisor'})
    assert approved.status_code==200,approved.text

    with api.app.state.sessions() as db:
        source_run=db.get(m.OptimisationRun,created.json()['id'])
        scope=api.app.state.live_demo.operational_scope(db,source_run,'P04')
    assert scope==['P01','P02','P03','P04']
