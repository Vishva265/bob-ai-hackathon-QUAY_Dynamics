...
def bounded_fraction(value):
    """Keep projected resource ratios inside their physical [0, 1] domain."""
    ...
    for hour in range(72):
        ...
        for (scope, scope_id), bids in berth_groups.items():
            ...
            row = dict(scope=scope, scope_id=scope_id, port_id=pid, terminal_id=tid,
                ...
                berth_utilisation=bounded_fraction(sum(occupied[bid] for bid in bids) / len(bids)),
                queue_length=count, average_wait_hours=sum(waits[bid] for bid in bids) / count if count else 0,
                yard_occupancy=bounded_fraction(sum(yard_mean[t] for t in tids) / sum(terminals[t]['yard_capacity_teu'] for t in tids)),
                crane_utilisation=bounded_fraction(sum(busy_cranes[bid] for bid in bids) / sum(available_cranes[bid] for bid in bids)) if sum(available_cranes[bid] for bid in bids) else 0,
