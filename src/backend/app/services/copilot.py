...
def infer_intent(question: str) -> str:
    ...
    if re.search(r'\b(approve|override|execute|delete|insert|update|ignore)\b', text):
    ...
    if re.search(r'\b(?:late|delay(?:ed)?|eta change|arrives?\b.*\b(?:hours?|h)|six hours|6\s*h)\b', text):
    ...
    if re.search(r'\b(?:at risk|risk|risky|highest wait|most delayed|waiting-time|waiting time)\b', text):
        return 'vessel_risk'
