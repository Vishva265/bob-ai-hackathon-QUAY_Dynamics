...
class LiveDemoManager:
    ...
    def forecast(self, db, as_of, port_id):
        return EarlyWarningService(db).run(EarlyWarningInput(as_of=as_of,port_ids=[port_id],predictor='ml'))
    ...
    def start(self,payload):
        ...
        try:
            with session_factory(branch).begin() as db:
                ...
                db.flush()
                forecast=self.forecast(db,origin,payload.port_id)
                run=PlanningService(db).run(OptimisationInput(as_of=origin,port_ids=[payload.port_id],predictor='ml',
                ...
                settings=dict(seed=payload.seed,time_limit_seconds=payload.time_limit_seconds,
                    ...
                    gate_policy='held_during_demo_ticks',model_version=forecast.model_version)
                run_id=run.id
            ...
            with session_factory(branch).begin() as db:
                ...
                if receipt:result=receipt.result
                    ...
                    def forecast_runner(payload):
                        ...
                        forecast=self.forecast(db,payload.as_of,demo.port_id)
