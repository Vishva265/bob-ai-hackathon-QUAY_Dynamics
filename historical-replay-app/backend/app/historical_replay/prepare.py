...
def derive(staging_database: Path, output: Path):
    ...
    try:
        ...
        for mmsi in ids:
            ...
            representative = max(track, key=lambda r: sum(v is not None and v != "" for v in r.values()))
            ...
            for sequence, group in enumerate(groups, 1):
                ...
                arrival = min((s[0] for s in waiting_points), default=group[0][0])
                ...
                calls.append({"id": call_id, "vessel_id": f"AIS-{mmsi}", "terminal_id": terminal,
        ...
        provenance = {"source": "NOAA Marine Cadastre 2021 daily AIS CSV", "real_fields": list(RAW_FIELDS),
            ...
            "limits": ["AIS cargo codes do not reliably identify container ships; only calls spatially associated with calibrated container-terminal proxies are treated as container candidates.", "Berth occupancy is a spatial proxy, not terminal operating-system ground truth."]}
