...
type Props={data:Dashboard;port:string;busy:boolean;actor:string;setActor:(v:string)=>void;onReview:()=>void;onApprove:()=>void;onDraft:()=>void;onExport:(f:'json'|'csv'|'html')=>void;onAck:(id:string,rev:number)=>void;onVessel:(id:string)=>void};

export function Shifts({data,port,busy,actor,setActor,onReview,onApprove,onDraft,onExport,onAck,onVessel}:Props){
  ...
  return <>
    <Panel title="A clear plan. A confident handover." eyebrow="ROLLING SUPERVISOR PUBLICATION" action={<Badge level={plan.status}/> }>
      ...
      <div className="context-notice"><TriangleAlert size={17}/><span>{data.run.scenario_id?'This is a hypothetical scenario. Review and approval apply only to operational drafts.':'Review changes and reconcile current conditions before approving dispatch.'}</span><button className="text-action" disabled={busy} onClick={onDraft}>Create operational draft →</button></div>
