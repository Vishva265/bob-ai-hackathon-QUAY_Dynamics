...
it('hides operator access when the server requires no key',async()=>{
  vi.stubGlobal('fetch',vi.fn(async()=>new Response(JSON.stringify({required:false,authenticated:true}))));
  render(<OperatorAccess onAuthenticated={vi.fn()}/>);
  await waitFor(()=>expect(screen.queryByRole('button',{name:'Operator access'})).not.toBeInTheDocument());
});
